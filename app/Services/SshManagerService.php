...
class SshManagerService
{
     ...
     */
    public function createHost(string $name, string $host, int $port = 22, string $user = 'root', ?string $skey = null): bool
    {
        $sshDir = $this->getSshDir();
    ...
    public function createKey(string $name, string $comment = '', string $password = ''): array
    {
        ...
        $command = "ssh-keygen -o -a 100 -t ed25519 -f " . escapeshellarg($keyPath) . " -C " . escapeshellarg($comment);
