        ...
        return $table
            ...
            ->actions([
                ...
                TableAction::make('save_to_config')
                    ...
                    ->modalDescription('This will create or update the corresponding SSH config file in ~/.ssh/config.d/')
                    ->action(function (SshConfig $record): void {
                        try {
                            ...
                            \Filament\Notifications\Notification::make()
                                ...
                                ->body("SSH configuration for '{$record->name}' has been saved to the config file.")
                                ->success()
                                ->send();
                        } catch (\Exception $e) {
                            \Filament\Notifications\Notification::make()
                ...
                TableAction::make('test_connection')
                    ...
                    ->action(function (SshConfig $record): void {
                        try {
                            ...
                            $ssh = \Spatie\Ssh\Ssh::create($record->username, $record->host, $record->port);
                            ...
                            if ($process->isSuccessful()) {
                                ...
                                throw new \Exception("Connection test failed: " . $process->getErrorOutput());
                            }
                        } catch (\Exception $e) {
                            \Filament\Notifications\Notification::make()
                ...
                Tables\Actions\DeleteAction::make()
                    ->requiresConfirmation()
                    ->modalDescription('This will delete the connection from the database. The actual SSH config file in ~/.ssh/config.d/ will not be deleted.'),
            ])
            ->bulkActions([
